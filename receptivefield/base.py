...
class ReceptiveField(metaclass=ABCMeta):
    ...
    def _get_gradient_activation_at_map_center(
        ...
        return self._get_gradient_from_grid_points(
            points=points,
            intensity=intensity
        )
    ...
    def compute(
            ...
            input_layer: str,
            output_layers: List[str]
    ) -> List[ReceptiveFieldDescription]:
        """
        ...
        :return a list of estimated ReceptiveFieldDescription for each feature
            map.
        """
        ...
        for fm, (rf_at_point00, rf_at00, rf_at11) in \
                enumerate(zip(rfs_at_point00, rfs_at00, rfs_at11)):
            ...
            _logger.info(
                f"Estimated receptive field for "
                f"feature map [{fm}]: {rf_params}")
    ...
    def plot_gradient_at(
        ...
        for fm in range(self.num_feature_maps):
            axis = plt.subplot(self.num_feature_maps, 1, fm + 1)
            plot_gradient_field(
                receptive_field_grad=receptive_field_grads[fm],
                image=image,
                axis=axis)

    def plot_gradient_at_feature_map(
            self,
            fm_id: int,
            point: GridPoint,
            image: Optional[np.ndarray] = None,
